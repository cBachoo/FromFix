...
// ---- SekiroFix fork: verify the extended xmm0-15 save/restore ----
#[cfg(test)]
use std::sync::atomic::{AtomicU32, Ordering as AtOrdering};

#[cfg(test)]
static CAP_XMM4: AtomicU32 = AtomicU32::new(0);
#[cfg(test)]
static CAP_XMM5: AtomicU32 = AtomicU32::new(0);
#[cfg(test)]
static CAP_XMM7: AtomicU32 = AtomicU32::new(0);

// A probe that loads the (win64 first) argument into xmm4/xmm5/xmm7, exposes a
// hook point, then returns xmm5's low dword. xmm7 (nonvolatile) is saved/restored.
#[cfg(test)]
std::arch::global_asm!(
    ".global sekirofix_xmm_probe",
    ".global sekirofix_xmm_probe_hook",
    "sekirofix_xmm_probe:",
    "  sub rsp, 16",
    "  movups [rsp], xmm7",
    "  movd xmm4, ecx",
...
#[cfg(test)]
unsafe extern "win64" {
    fn sekirofix_xmm_probe(x: u32) -> u32;
    static sekirofix_xmm_probe_hook: u8;
}

#[cfg(test)]
unsafe extern "win64" fn xmm_capture_handler(reg: *mut Registers, _: usize) {
    let r = &mut *reg;
    CAP_XMM4.store(r.xmm4 as u32, AtOrdering::Relaxed);
    CAP_XMM5.store(r.xmm5 as u32, AtOrdering::Relaxed);
    CAP_XMM7.store(r.xmm7 as u32, AtOrdering::Relaxed);
    // Modify xmm5's low dword; the probe should return the new value.
    let nv = (r.xmm5 as u32).wrapping_add(0x1000);
    r.xmm5 = (r.xmm5 & !0xFFFF_FFFFu128) | nv as u128;
}

#[test]
fn test_xmm4_5_7_roundtrip() {
    const INPUT: u32 = 0x1234_5678;
    // Baseline: no hook -> returns input unchanged.
    assert_eq!(unsafe { sekirofix_xmm_probe(INPUT) }, INPUT);

    let hook_addr = unsafe { &sekirofix_xmm_probe_hook as *const u8 as usize };
    let hooker = Hooker::new(
        hook_addr,
        HookType::JmpBack(xmm_capture_handler),
        CallbackOption::None,
        0,
        HookFlags::empty(),
    );
    ...
    let ret = unsafe { sekirofix_xmm_probe(INPUT) };

    // The handler saw the correct values in xmm4/5/7...
    assert_eq!(CAP_XMM4.load(AtOrdering::Relaxed), INPUT, "xmm4 not captured");
    assert_eq!(CAP_XMM5.load(AtOrdering::Relaxed), INPUT, "xmm5 not captured");
    assert_eq!(CAP_XMM7.load(AtOrdering::Relaxed), INPUT, "xmm7 not captured");
    // ...and the write-back to xmm5 took effect in the running code.
    assert_eq!(ret, INPUT.wrapping_add(0x1000), "xmm5 write-back failed");

    unsafe { info.unhook().unwrap() };
    // After unhook, back to baseline.
    assert_eq!(unsafe { sekirofix_xmm_probe(INPUT) }, INPUT);
}
