...
/// Emit a `movaps` between `xmmN` and `[rsp+disp]`.
/// `store == true` emits `movaps [rsp+disp], xmmN` (opcode 0F 29),
/// otherwise `movaps xmmN, [rsp+disp]` (opcode 0F 28). `disp` is `i as i32 * 16`.
fn emit_movaps(out: &mut Vec<u8>, store: bool, i: u8) {
    let disp = (i as i32) * 16;
    if i >= 8 {
        out.push(0x44); // REX.R for xmm8-15
    }
    out.push(0x0F);
    out.push(if store { 0x29 } else { 0x28 });
    let reg = i & 7;
    // base = rsp -> rm = 100 requires a SIB byte (0x24 = scale 0, no index, base rsp)
    if disp == 0 {
        out.push((reg << 3) | 0b100); // mod 00
        out.push(0x24);
    } else if disp < 128 {
        out.push((0b01 << 6) | (reg << 3) | 0b100); // mod 01, disp8
        out.push(0x24);
        out.push(disp as u8);
    } else {
        out.push((0b10 << 6) | (reg << 3) | 0b100); // mod 10, disp32
        out.push(0x24);
        out.extend_from_slice(&disp.to_le_bytes());
    }
}

/// Emit `save`/`restore` for xmm0-xmm15 into the 0x100-byte scratch area at `[rsp]`.
fn emit_xmm_block(out: &mut Vec<u8>, store: bool) {
    for i in 0..16u8 {
        emit_movaps(out, store, i);
    }
}
...
fn write_trampoline_prolog(buf: &mut impl Write) -> Result<usize, std::io::Error> {
    ...
    // ...
    // movaps xmmword ptr ss:[rsp+0xF0],xmm15
...
fn write_trampoline_epilog1(buf: &mut impl Write) -> Result<usize, std::io::Error> {
    ...
    // ...
    // movaps xmm15,xmmword ptr ss:[rsp+0xF0]
    // add rsp,0x100
    // pop r15
    // pop r14
    // pop r13
...
fn generate_retn_trampoline<T: Write + Seek>(
    ...
    // call rax
    // add rsp, 0x20
    // mov [rsp + 0x188], rax  (rax slot; shifted by the larger 0x100 xmm block)
    buf.write(&[
        0xff, 0xd0, 0x48, 0x83, 0xc4, 0x20, 0x48, 0x89, 0x84, 0x24, 0x88, 0x01, 0x00, 0x00,
    ])?;
    write_trampoline_epilog1(buf)?;
    write_trampoline_epilog2_common(buf)?;
    // ret
